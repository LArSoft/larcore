...
public:
  struct GeometryInfoConfig {
    ...
    fhicl::Atom<bool> Required{fhicl::Name{"Required"},
  ...
  struct Config {
    ...
    fhicl::OptionalTable<GeometryInfoConfig> GeometryLegacyInfo{
      ...
      fhicl::Comment{"Information to check about geometry (legacy from RunData)"}};
...
namespace {
  ...
  bool case_insensitive_equal(std::string const& a, std::string const& b)
  {
    ...
    return std::equal(a.begin(), a.end(), b.begin(), b.end(), [](auto a, auto b) {
  ...
  , fLegacyCheckInfo(makeGeometryInfoCheckInfo(config().GeometryLegacyInfo))
{
  ...
  if (fCheckInfo) consumes<sumdata::GeometryConfigurationInfo>(GeometryConfigurationWriterTag);
  ...
  // --- BEGIN -- Configuration dump -------------------------------------------
  {
    ...
    if (fCheckInfo) {
      if (fCheckInfo->detectorName.empty())
        log << " (any name)";
      else
        log << " must match '" << fCheckInfo->detectorName << "'";
    ...
    if (fLegacyCheckInfo) {
      if (fLegacyCheckInfo->detectorName.empty())
        log << " (any name)";
      else
        log << " must match '" << fLegacyCheckInfo->detectorName << "'";
...
void geo::GeometryInfoCheck::CheckGeometryInfo(art::Run const& run,
                                               GeometryInfoCheckInfo const& config) const
{
  ...
  if (!run.getByLabel(GeometryConfigurationWriterTag, hInfo)) {
    ...
    throw cet::exception("GeometryInfoCheck") << "Required geometry information not found as '"
  ...
  if (!config.detectorName.empty() &&
    ...
    throw cet::exception("GeometryInfoCheck")
      << "Geometry information reports an unexpected name '" << info.detectorName << "' ('"
      << config.detectorName << "' expected)\n";
  } // if
                                                     ...
                                                     GeometryInfoCheckInfo const& config) const
{
  ...
  if (hInfoList.empty()) {
    ...
    log << "\nUnfortunately, that was required...";
    throw cet::exception("GeometryInfoCheck") << "No legacy geometry information found!\n";
  } // if no info
  ...
  if (!config.detectorName.empty() &&
    ...
    throw cet::exception("GeometryInfoCheck")
      << "Geometry legacy information reports an unexpected name '" << info.DetName() << "' ('"
      << config.detectorName << "' expected)\n";
  } // if
...
auto geo::GeometryInfoCheck::makeGeometryInfoCheckInfo(
  fhicl::OptionalTable<GeometryInfoConfig> const& config) -> std::optional<GeometryInfoCheckInfo>
{
